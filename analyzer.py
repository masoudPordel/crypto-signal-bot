...
async def analyze_symbol(exchange: ccxt.Exchange, symbol: str, tf: str) -> Optional[dict]:
    ...
    try:
        ...
        df = await get_ohlcv_cached(exchange, symbol, tf, limit=200)  # افزایش برای MA
        ...
        # اضافه کردن امتیازات توابع جدید
        usdt_dominance_series = pd.Series(np.random.uniform(0.4, 0.6, len(df)))  # نمونه موقت
        ...
        if score_long >= THRESHOLD and trend_1d_score >= 0:
            ...
            result = {
                ...
                "روند بازار": "صعودی",
                "فاندامنتال": f"امتیاز: {score_log['long']['fundamental']}",
